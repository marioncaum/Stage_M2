...
\begin{prop}
Cet algorithme est sans-envie up to one slot.
\end{prop}

\begin{proof}
Soit $p$ un ordre sur les agents. Soient $i, j \in \{1, \dots, n\}$. On note $s_i^t$ le slot alloué à l'agent $i$ au tour $t$, pour $t\in\{1,\dots,x\}$.\\
\textbf{Cas 1 : $i<j$}\\
\\
\textbf{Cas 2 : $i>j$}
\end{proof}
